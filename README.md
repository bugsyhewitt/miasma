...
| `miasma_jupyter_001` | `MIASMA-JUPYTER-001` | Jupyter Notebook or JupyterLab reachable without authentication — `GET /api` fingerprints Jupyter via the `version` key in the response JSON (present since Notebook 4.x / JupyterLab 1.x); `GET /api/kernels` returning 200 confirms kernel-management access without a token, which is a direct RCE vector (`POST /api/kernels` + WebSocket `execute_request` executes arbitrary code, CRITICAL); `GET /api/kernelspecs` returning 200 without auth enumerates the execution surface (HIGH); `GET /api` accessible with both sub-endpoints gated is MEDIUM. No kernel is created, no code is executed, no file is read. Probe ports: 8888 (primary), 8889, 8890, 8080, 10000, 80, 443. |
| `miasma_activemq_001` | `MIASMA-ACTIVEMQ-001` | Apache ActiveMQ admin console reachable with the factory `admin:admin` credential or without authentication — `GET /` fingerprints ActiveMQ via the `"ActiveMQ"` marker in the response body; a 200 without an auth challenge is a HIGH finding; a 401 triggers a single `admin:admin` default-credential attempt (CRITICAL if accepted); unauthenticated Jolokia REST API at `/api/jolokia/` leaking broker name and queue topology is MEDIUM. Related to CVE-2023-46604 (CVSS 10.0, pre-auth RCE). Probe ports: 8161 (primary), 80, 443, 8080. No queue created/deleted, no message published/consumed, no configuration changed. |
