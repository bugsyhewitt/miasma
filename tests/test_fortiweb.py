"""Tests for the Fortinet FortiWeb auth-bypass probe (CVE-2025-64446).
...
(see tests/test_tomcat_rewrite.py and tests/test_actuator.py).
"""
...
PLUGIN = "cve_2025_64446"

# A realistic privileged status JSON body (carries the markers the probe checks).
_STATUS_JSON = (
    '{"http_method":"GET","results":{"serial":"FVVM01TM0000ABCD",'
    '"version":"7.6.3","build":"0123"},"status":"success"}'
)

# FortiWeb login page body marker.
_FORTIWEB_LOGIN = "<title>FortiWeb</title><body>Please Login</body>"
_FORTIWEB_SERVER = "xxxxxxxx-xxxxx"  # FortiWeb often blanks Server; rely on body
...
def _make_fake_get(path_map: dict[str, httpx.Response], record: list | None = None):
    ...
    Keys are matched against the decoded ``urlparse`` path.
    """
...
def _target() -> Target:
    return Target(host="10.0.0.5", ports={443: {"state": "open", "name": "https"}})


def test_plugin_is_discoverable_and_valid():
    assert PLUGIN in available_plugins()
    module = load_plugin(PLUGIN)
    assert module.metadata["vuln_id"] == "CVE-2025-64446"
    assert callable(module.probe)


def test_confirmed_bypass_returns_high(monkeypatch):
    """Privileged status via traversal while the direct path refuses → HIGH."""
    module = load_plugin(PLUGIN)

    def fake_get(url, *args, **kwargs):
        path = urlparse(url).path
        if path == "/login":
            return _resp(200, _FORTIWEB_LOGIN)
        # Direct authenticated endpoint refuses without a session.
        if path == "/api/v2.0/cmdb/system/status":
            return _resp(401, "unauthorized")
        # ...but a traversal shape leaks the privileged status JSON.
    ...
    assert finding.vuln_id == "CVE-2025-64446"
    assert finding.confidence == "high"
    assert finding.host == "10.0.0.5"
    assert finding.evidence["direct_status"] == 401
    assert finding.evidence["traversal_status"] == 200
    assert "status" in finding.evidence["traversal_path"].lower()


def test_fortiweb_via_server_header(monkeypatch):
    ...
    def fake_get(url, *args, **kwargs):
        path = urlparse(url).path
        if path == "/":
            return _resp(200, "ok", headers={"server": "FortiWeb"})
        if path == "/api/v2.0/cmdb/system/status":
            return _resp(403)
        if "status" in url.lower():
            return _resp(200, _STATUS_JSON)
...
def test_fortiweb_but_traversal_blocked_returns_none(monkeypatch):
    """FortiWeb fingerprinted but every traversal path is correctly blocked → none."""
    module = load_plugin(PLUGIN)
    ...
    def fake_get(url, *args, **kwargs):
        ...
        if path == "/api/v2.0/cmdb/system/status":
            return _resp(401)
        # Every traversal attempt is correctly rejected.
        return _resp(403)
...
def test_fortiweb_suspicious_200_without_markers_is_medium(monkeypatch):
    """Traversal returns a 200 with no status markers → MEDIUM, not HIGH."""
    module = load_plugin(PLUGIN)
...
def test_fortiweb_direct_not_refusing_is_medium(monkeypatch):
    """Traversal leaks status but the direct path didn't refuse → MEDIUM (no clean control)."""
    module = load_plugin(PLUGIN)
    ...
    def fake_get(url, *args, **kwargs):
        ...
        # clean CVE-2025-64446 bypass control. Probe must not claim HIGH.
        if path == "/api/v2.0/cmdb/system/status":
            return _resp(200, _STATUS_JSON)
        if "status" in url.lower():
            return _resp(200, _STATUS_JSON)
...
def test_non_fortiweb_host_returns_none(monkeypatch):
    """A non-FortiWeb host with odd 200s must not be flagged (no false positive)."""
    module = load_plugin(PLUGIN)
    ...
    def fake_get(url, *args, **kwargs):
        ...
        # Even a 200 here must not flag — not FortiWeb.
        return _resp(200, _STATUS_JSON)
...
def test_run_through_runner_collects_finding(monkeypatch):
    ...
    assert findings[0].vuln_id == "CVE-2025-64446"
    assert findings[0].confidence == "high"


@pytest.mark.parametrize(
    "port,expected_scheme",
    [(443, "https"), (8443, "https"), (80, "http")],
)
